							...
							<table id="table_orderinfo">
								...
								<c:forEach var="orderInfo" items="${orderList}">
									<c:forEach var="commodityInfo" items="${commodityList}">
										<c:if test="${user.getUser_phone() == orderInfo.getOrder_beneficiary_phone()}">
											<c:if test="${orderInfo.getCommodity_id() == commodityInfo.getCommodity_id()}">
												<tr>
													...
													<td>
														<c:if test="${orderInfo.getOrder_commodity_time_length() == 0}">
															今年
														</c:if>
														<c:if test="${orderInfo.getOrder_commodity_time_length() != 0}">
															${orderInfo.getOrder_commodity_time_length()}年間
														</c:if>
													</td>
												</tr>
											</c:if>
