 ...
 * @Description: 商品データ層のインターフェースの実装クラス
 * @author JY
 */
public class CommodityDaoImpl implements CommodityDao{
	...
	public List<Commodity_liability_info> selectAllCommodity(MyUserInfo myUserInfo,String commodityType) {
		Integer commodityTypeInt = 0;
		if (commodityType != null) {
			commodityTypeInt = Integer.parseInt(commodityType);
		}
		Integer pd = 0;
		...
		if(commodityTypeInt == 0) {
			pd = 0;
		} else {
			sql = sql + " WHERE c1.c1 = " + commodityTypeInt;
			pd = 1;
		}
		String sql2 = sql;
		String sql3 = "";
		...
		if(myUserInfo != null) {
			if (pd == 0) {
				if("先生".equals(myUserInfo.getUser_sex())) {
				...
				if("女士".equals(myUserInfo.getUser_sex())) {
					sql2 = sql + " AND c1.commodity_applicable_gender in(1,2) ";
					sql3 = sql + " AND c1.commodity_applicable_gender in (0,2) ";
				}else {
					sql2 = sql + " AND c1.commodity_applicable_gender in(0,2) ";
					sql3 = sql + " AND c1.commodity_applicable_gender in(1,2) ";
				}
			}
	...
	/**
	 * @Title: fl
	 * @Description: 取得したSQLデータベース照会文を使用して対応する商品情報を照会する
	 * @param @param sql
	 * @param @return  
	 * @return List<Commodity_liability_info>    
	 * @throws
	 */
