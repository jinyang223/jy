 ...
 * @Description: ユーザーデータ層インターフェースの実装クラス
 * @author JY
 */
public class UserDaoImpl implements UserDao{
	...
	/**
	 * @Title: selectUserIdByEmail
	 * @Description: ユーザーのメールアドレスに基づいてユーザーIDを取得する
	...
	public boolean upDataUserInfo(Integer userInfoId,String user_pass,String user_phone,Double addMoneyDouble) {
		String sql1 = "UPDATE my_user SET user_password = ? WHERE user_id = (SELECT user_id FROM my_user_info WHERE user_info_id = ?)";
		String sql2 = "UPDATE my_user_info SET user_phone = ? ,user_money = user_money + ? WHERE user_id = ?";
		boolean bo = false;
		try {
			bo = JDBCUTILE.upDataBaseTable(sql1, user_pass,userInfoId);
			if(bo) {
				bo = JDBCUTILE.upDataBaseTable(sql2, user_phone,addMoneyDouble,userInfoId);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
